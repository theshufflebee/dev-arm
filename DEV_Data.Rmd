...
dataset_deciles = left_join(dataset_deciles, agric_data, by = c("year", "district"))

dataset_quartiles = left_join(dataset_quartiles, agric_data, by = c("year", "district"))
...
plot2_data_q <- dataset_prepped_q %>%
  ...
  summarize(avg_income = mean(income, na.rm = TRUE),
            avg_agr_income = mean(agric_income, na.rm = TRUE),
            mean_prod = mean(`Gross  agricultural  output, total`, na.rm = TRUE), .groups = 'drop')
...
ggplot(dataset_prepped_q, aes(x = drought_status, y = agric_income/income, fill = drought_status)) +
  ...
  theme(legend.position = "none", 
        ...
        plot.title = element_text(size = 11, face = "bold", hjust = 0.5))
```
```{r desc ev drought on agri prod by quartile, message=FALSE, warning=FALSE}

# D2. 
ggplot(plot2_data_q, aes(x = year, y = mean_prod, color = drought_status, group = drought_status)) +
  geom_line(linewidth = 1.1, alpha = 0.9) +
  ...
  scale_color_manual(values = c("No Drought Event" = "#0072B2", "Drought Event" = "#D55E00")) +
  labs(
    title = "Impact of Drought Events on Agriculture Production, by Income Quartile",
    subtitle = "Average Production trends faceted by income group",
    x = "Year",
    y = "Average Annual Production",
    color = "Drought Status") +
  theme_bw() + # A clean theme
  theme(
    legend.position = "top",
...
```{r desc ev drought on agri prod by decile, message=FALSE, warning=FALSE}

# D2. Create the faceted plot q
ggplot(plot2_data_d, aes(x = year, y = mean_prod, color = drought_status, group = drought_status)) +
  geom_line(linewidth = 1.1, alpha = 0.9) +
  
  # Create 4 separate plots, one for each 'income_decile'
  facet_wrap(~ income_decile, scales = "free_y") +
  ...
  scale_color_manual(values = c("No Drought Event" = "#0072B2", "Drought Event" = "#D55E00")) +
  labs(
    title = "Impact of Drought Events on Agriculture Production, by Income Decile",
    subtitle = "Average Production trends faceted by income group",
    x = "Year",
    y = "Average Annual Production",
    color = "Drought Status") +
