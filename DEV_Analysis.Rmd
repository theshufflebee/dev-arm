...
# For numbering of captions to make it easier to find ourselves in latex
section_base <- 4 
dv_counter <- 2
...
for (dv in dependent_vars) {
  ...
  for (group_name in names(iv_groups)) {
    ...
    section_str <- paste(section_base, dv_counter, iv_counter, sep = ".")
    ...
    dynamic_caption <- sprintf("Regression results for %s on %s", 
                               dv_safe, group_name)
    
    if (latex_format) {
    etable(models_list,
      title = dynamic_caption,
      label = paste0("table",section_str),
      fixef_sizes = TRUE,     
      fitstat = c("n", "r2", "wr2"), 
      tex = latex_format, 
      file = file_path,
