...
twfe_data$crops_output = log(twfe_data$crops_output)
...
# Define dependent & independent variables and model names
dependent_vars <- c("income", "agric_income", "agric_output", "crops_output",
...
iv_formulas <- c( "spei", "spei + spei_lag1","spei + spei_lag1 + spei_lag2",
                  "share", "share_lag1", "share_lag2",
                  "agric_stress", "agric_stress_lag1", "agric_stress_lag2")
model_names <- c( "Model 1: TWFE with 0 Lags",
                  "Model 2: TWFE with 1 Lag",
                  "Model 3: TWFE with 2 Lags")

# Loop over each variable
for (dv in dependent_vars) {
      models_list <- list()
  models_list[[1]] <- feols(
    as.formula(sprintf("%s ~ %s | district + year", dv, iv_formulas[1])),
    data = twfe_data,
    cluster = ~ district)
  
  models_list[[2]] <- feols(
    as.formula(sprintf("%s ~ %s | district + year", dv, iv_formulas[2])),
    data = twfe_data,
    cluster = ~ district)
  
  models_list[[3]] <- feols(
    as.formula(sprintf("%s ~ %s | district + year", dv, iv_formulas[3])),
    data = twfe_data,
    cluster = ~ district)
  
  # Name models and print
  names(models_list) <- model_names
  cat(paste0("\n\n", "---", "\n\n")) # Adds a separator
  cat(paste0("## Regression Table for: ", dv, "\n\n"))
  print(etable(models_list)) }
...
for (dv in dependent_vars) {
  cat(paste0("\n\n", "========================================", "\n"))
  cat(paste0("## Analyzing Dependent Variable: ", dv, "\n"))
  cat(paste0("========================================", "\n"))
  for (group_name in names(iv_groups)) {
    ...
    # This loop runs 3 times (for 0, 1, and 2 lags)
    ...
    table_title <- paste0("DV: '", dv, "'  |  IVs: '", group_name, " Lags'")
    ...
    print(etable(
      ...
      fitstat = c("n", "r2"), 
      notes = c(
        "All models include District and Year fixed effects.",
...
```{r TWFE regression stress, message=FALSE, warning=FALSE}
...
```{r TWFE regression dummy, message=FALSE, warning=FALSE}
...
```{r TWFE regression dummy2, message=FALSE, warning=FALSE}
